...
run_ITS_model_multi <- function(df,
                                ...
                                plot_title = NULL,       # New argument for plot title
                                y_axis_label = NULL) {   # New argument for y-axis label
  ...
  ts_data <- DF %>%
    dplyr::transmute(
      ...
      fitted = predicted_full,
      counterfactual = counterfactual_pred
    )
  
  # Defaults for plot title and y-axis label
  if (is.null(plot_title)) {
    plot_title <- paste("ITS with Multiple Interruptions:", organism_name)
  }
  if (is.null(y_axis_label)) {
    y_axis_label <- ifelse(pop_include, "Incidence Rate", "Count")
  }
