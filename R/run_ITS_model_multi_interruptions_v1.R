...
run_ITS_model_multi <- function(df,
  ...
  # Plot with footnote explaining the three lines
  footnote_text <- paste(
  ...
  plot <- ggplot(DF, aes(x = date_month)) +
    ...
    geom_vline(xintercept = interruptions$start, linetype = "dotted") +
    geom_vline(xintercept = interruptions$end, linetype = "dashed") +
    labs(
